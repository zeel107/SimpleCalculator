...
def divide_str(str, index, operator):
...
def calculate(str):
    ...
    while(i < len(str)):
        ...
        if(str[i] == ')'):
            ...
            i+=1
            continue
        if(operators.count(str[i]) > 0 and paren == 0):
            if(get_order(str[i]) >= order):
                operator = str[i]
                index = i
                order = get_order(str[i])
                i+=1
                if(operator == 'c'):
                    i += 2
                continue
    ...
    if(operator == ''):         # there is no operator, so the str may be inclosed with (), so remove those and return the answer
        if(str.count('(') > 0):
            # remove the ()
            return calculate(remove_paren(str))
        else:
            return int(str)
    ...
    split = divide_str(str, index, operator)
    ...
    if (operator == '^'):
        return calculate(split[0]) ** calculate(split[1])
    if (operator == 'sin'):
        return math.sin(calculate(split[1]))
    if (operator == 'cos'):
        return math.cos(calculate(split[1]))
    if(operator == 'cot'):
        return 1 / math.tan(calculate(split[1]))
    if (operator == 'tan'):
        return math.tan(calculate(split[1]))
    if (operator == 'ln'):
        return math.log(calculate(split[1]), math.e)
    if(operator == 'log'):
        return math.log(calculate(split[1]), 10)


print(calculate("sin(4*(4^ln(7)))"))
print(calculate("(2^(2^(2^2)))^2"))
